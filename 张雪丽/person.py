...
# 人口数量excel文件保存路径
POPULATION_EXCEL_PATH = 'E:\Big_Data\shuju.xlsx'
def spider_all_population():
    ...
    get_population_info(population_dict, response1.json(),strict=False)

    response2 = requests.get(url.format(dfwds2),verify=False)
    get_population_info(population_dict, response2.json(),strict=False)

    response3 = requests.get(url.format(dfwds3),verify=False)
    get_population_info(population_dict, response3.json(),strict=False)
